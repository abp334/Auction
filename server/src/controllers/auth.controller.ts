...
import { comparePassword, hashPassword, signAccessToken, signRefreshToken, verifyRefreshToken, hashRefreshToken, compareRefreshToken } from '../utils/auth';
...
export async function signup(req: Request, res: Response) {
  ...
  if (existing) return res.status(StatusCodes.CONFLICT).json({ error: 'Email already registered' });

  // Validate team exists for captains
  if (value.role === 'captain' && value.teamId) {
    const team = await Team.findById(value.teamId);
    if (!team) return res.status(StatusCodes.BAD_REQUEST).json({ error: 'Selected team does not exist' });
    
    // Check if team already has a captain
    const existingCaptain = await User.findOne({ teamId: value.teamId, role: 'captain' });
    if (existingCaptain) return res.status(StatusCodes.CONFLICT).json({ error: 'Team already has a captain' });
  }
  ...
  return res.status(StatusCodes.CREATED).json({
    accessToken: at,
    user: { id: created.id, email: created.email, name: created.name, role: created.role, teamId: created.teamId },
  });
}
...
export async function me(req: Request & { user?: { id: string; role: string } }, res: Response) {
...
export async function refresh(req: Request, res: Response) {
  ...
  if (!rt) return res.status(StatusCodes.UNAUTHORIZED).json({ error: 'Missing refresh token' });
  ...
  if (!user) return res.status(StatusCodes.UNAUTHORIZED).json({ error: 'Invalid session' });
  const ok = await compareRefreshToken(rt, user.refreshTokenHash);
  if (!ok) return res.status(StatusCodes.UNAUTHORIZED).json({ error: 'Invalid session' });
